...
class Conjunction(MappedAnimationBase):
    ...
    def __init__(self, leds, **kwargs):
        ...
        if not self.generator:
            ...
            self.generator = generators.fading_color_wheel(
                hue_stride=25, stripe_size=16, mode=trickLED.FADE_OUT)
        self.palette = trickLED.ByteMap(33, bpi=3)

    def setup(self):
        self.state = {'step': 0, 'insert_points': []}
        self._ordered_palette = trickLED.ByteMap(
            self.palette.n, self.palette.bpi)
        self.start_cycle()
