...
def randrange(low, high):
    ...
    difference = high - low
    bit_length = len(bin(difference)) - 2
    random_number = getrandbits(bit_length) + low
    if low <= random_number <= high:
        return random_number
    return_value = random_number & high
    if low <= return_value <= high:
        return return_value
    return_value = random_number | low
    if low <= return_value <= high:
        return return_value
    return high
...
class ByteMap:
    ...
    def add(self, val):
        if isinstance(val, (list, tuple)):
            if len(val) < self.bpi:
                raise ValueError(
                    'Length of value to add must match byte size.')
            self.buf = bytearray([uint8(self.buf[i] + val[i % self.bpi])
                                 for i in range(self.n * self.bpi)])
        else:
    ...
    def sub(self, val):
        if isinstance(val, (list, tuple)):
            if len(val) < self.bpi:
                raise ValueError(
                    'Length of value to sub must match byte size.')
            self.buf = bytearray([uint8(self.buf[i] - val[i % self.bpi])
                                 for i in range(self.n * self.bpi)])
        else:
            self.buf = bytearray([uint8(v - val) for v in self.buf])
    ...
    def mul(self, val):
        if isinstance(val, (list, tuple)):
            if len(val) < self.bpi:
                raise ValueError(
                    'Length of value to multiply must match byte size.')
            self.buf = bytearray([uint8(self.buf[i] * val[i % self.bpi])
                                 for i in range(self.n * self.bpi)])
        else:
            self.buf = bytearray([uint8(v * val) for v in self.buf])
    ...
    def div(self, val):
        if isinstance(val, (list, tuple)):
            if len(val) < self.bpi:
                raise ValueError(
                    'Length of value to divide must match byte size.')
            self.buf = bytearray(
                [uint8(self.buf[i] // val[i % self.bpi]) for i in range(self.n * self.bpi)])
        else:
            self.buf = bytearray([uint8(v // val) for v in self.buf])
...
class TrickLED(NeoPixel):
    ...
    def add(self, val):
        ...
        if isinstance(val, (list, tuple)):
            ...
            self.buf = bytearray([uint8(self.buf[i] + val[i % bpp])
                                 for i in range(mi * bpp)])
        else:
            self.buf = bytearray([uint8(self.buf[i] + val)
                                 for i in range(mi * bpp)])
