...
def default_palette(n, brightness=200):
    ...
    random_number = getrandbits(8)
    palette = trickLED.ByteMap(n, 3)
    sa = min(255 // n, 30)
    for i in range(n):
        palette[i] = trickLED.color_wheel(
            (random_number + sa * i) % 255, brightness
        )
    return palette
...
class AnimationBase:
    ...
    def palette(self, val):
        ...
        if isinstance(val, trickLED.ByteMap):
            ...
            raise ValueError('Invalid type for palette {}'.format(
    ...
    def _print_fps(self):
        start_ticks = self.state.get('start_ticks')
        if start_ticks is None:
            return
        end_ticks = time.ticks_ms()
        interval = self.settings.get('interval')
        fps = self.frame / time.ticks_diff(end_ticks, start_ticks) * 1000
        interval_fps = 1000 / interval if interval > 0 else 1000
        print(
            'Actual fps: {:0.02f} - interval fps: {:0.02f}\n'.format(fps, interval_fps))
...
class NextGen(AnimationBase):
    ...
    def __init__(self, leds, generator=None, blanks=0, scroll_speed=1, **kwargs):
        ...
        if generator is None:
            generator = generators.striped_color_wheel(
                hue_stride=10, stripe_size=1)
    ...
    def setup(self):
        ...
        if self.settings['scroll_speed'] not in (-1, 1):
            self.settings['scroll_speed'] = min(
                max(self.settings['scroll_speed'], -1), 1) or 1
    ...
    def setup(self):
        if self.palette is None:
            self.palette = default_palette(
                20, self.settings.get('brightness', 200))
...
class Jitter(LitBits):
    ...
    def setup(self):
        ...
        self.settings['background'] = trickLED.colval(
...
class SideSwipe(AnimationBase):
    ...
    def __init__(self, leds, color_generators=None, **kwargs):
        ...
        if color_generators:
            ...
            self.generators.append(generators.striped_color_wheel(
                hue_stride=20, stripe_size=10))
...
class Convergent(AnimationBase):
    ...
    def start_cycle(self):
        ...
        self.leds.fill((0, 0, 0))
        self.state['palette_idx'] = (
            self.state['palette_idx'] + 1) % len(self.palette)
